...
export default function App() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const saved = await AsyncStorage.getItem('my_tracker_final_perfect_v100');
        if (saved) {
  ...
  const saveBooks = async (updatedBooks) => {
    ...
    await AsyncStorage.setItem('my_tracker_final_perfect_v100', JSON.stringify(updatedBooks));
  };
  ...
  const handleSaveBook = () => {
    ...
    if (norm > total) {
      norm = total;
      Alert.alert("Внимание", `Норма в день не может превышать общее число страниц! Установлено: ${total} стр.`);
    }
  ...
  const handleDeleteBook = (id) => {
    Alert.alert(
      "Удаление книги",
      "Точно уверены, что хотите удалить книгу?",
      [
  ...
  const getDayCircleColor = (dayObj) => {
    if (!activeBook) return '#f2f2f7';
    const dateKey = formatDateKey(dayObj);

    if (dateKey > todayKey) return '#f2f2f7'; // Будущие дни наперед красить нельзя

    const progressForDay = activeBook.progress && activeBook.progress[dateKey] !== undefined 
      ? parseInt(activeBook.progress[dateKey], 10) 
      : 0;
    
    const norm = parseInt(activeBook.dailyNorm, 10) || 1;

    if (progressForDay === 0) return '#f2f2f7'; 
    if (progressForDay === norm) return '#007AFF'; // СИНИЙ — норма
    if (progressForDay > norm) return '#FFCC00';    // ЖЁЛТЫЙ — выше нормы
    return '#FF3B30';                              // КРАСНЫЙ — меньше нормы
  };
            ...
            return (
              <TouchableOpacity key={item.id} style={styles.bookCard} onPress={() => { setSelectedBookId(item.id); setScreen('calendar'); }}>
                <View style={{ flex: 1 }}>
                  ...
                  {itemLeft === 0 ? (
                    <Text style={styles.bookCardFinishedText}>Прочитано! Ура!</Text>
                  ) : (
  ...
  // --- ЭКРАН 2 & 3: ДОБАВЛЕНИЕ / РЕДАКТИРОВАНИЕ ---
      ...
      <View style={styles.container}>
        ...
        <View style={styles.formContainer}>
          <View style={styles.inputBlock}>
            ...
            <TextInput style={styles.borderInput} value={bookTitle} onChangeText={setBookTitle} underlineColorAndroid="transparent" />
          </View>
          
          <View style={styles.inputBlock}>
            <Text style={styles.inputPlaceholder}>Всего страниц</Text>
            <TextInput style={styles.borderInput} keyboardType="numeric" value={totalPages} onChangeText={(text) => setTotalPages(text.replace(/[^0-9]/g, ''))} underlineColorAndroid="transparent" />
          </View>
          
          <View style={styles.inputBlock}>
            <Text style={styles.inputPlaceholder}>Норма в день</Text>
            <TextInput style={styles.borderInput} keyboardType="numeric" value={dailyNorm} onChangeText={(text) => setDailyNorm(text.replace(/[^0-9]/g, ''))} underlineColorAndroid="transparent" />
          </View>
          ...
          <TouchableOpacity style={styles.cancelLink} onPress={() => { setBookTitle(''); setTotalPages(''); setDailyNorm(''); setScreen('library'); }}>
      ...
      <View style={styles.container}>
        ...
        <View style={styles.weekContainer}>
          {weekDays.map((day, idx) => {
            ...
            const circleColor = getDayCircleColor(day);
            ...
            return (
              <TouchableOpacity 
                ...
                style={[
                  ...
                  isActualToday && styles.todayGreenCircle
                ]} 
        ...
        <Text style={styles.statusMessage}>
          {isFuture ? 'ПИСАТЬ НАПЕРЁД НЕЛЬЗЯ!' : 'СКОЛЬКО ПРОЧИТАЛ СЕГОДНЯ?'}
        </Text>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#fff', paddingHorizontal: 25, paddingTop: 50, alignItems: 'center' },
  ...
  bookCard: { backgroundColor: '#f7f7f7', padding: 18, borderRadius: 16, flexDirection: 'row', alignItems: 'center', marginBottom: 12 },
  ...
  todayGreenCircle: { borderWidth: 2.5, borderColor: '#006400' }, 
